...
$query = "SELECT s.book, s.withdrawal, s.created_at, c.name, f.field_name, cn.center_name, p.period_name  FROM saving_withdrawals AS s INNER JOIN client_registers AS c ON s.client_id = c.client_id INNER JOIN feilds AS f ON f.feild_id = s.feild_id INNER JOIN centers AS cn ON cn.center_id = s.center_id INNER JOIN periods AS p ON p.period_id = s.period_id WHERE s.status = '1' AND s.officers_id = '${officer_id}' AND DATE_FORMAT(s.created_at, '%Y-%m-%d') BETWEEN '${from_date}' AND '${end_date}' ORDER BY s.withdraw_id DESC";
